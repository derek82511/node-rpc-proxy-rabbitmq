...
function RPCClient(host, port, logger, func) {
    this.mq = {
        host: host,
        port: port,
        connection: null,
    ...
    let instanceId = typeof (process.env.pm_id) === 'undefined' ? 'default' : process.env.pm_id

    this.rpcQueues = {
        requestQueueName: `${func}-request-queue`,
        replyQueueName: `${func}-reply-queue-${os.hostname()}-${instanceId}`
    }
...
function build(options) {
    ...
    return new RPCClient(options.host, options.port, options.logger, options.func)
}
